...
import { CardReissuanceReq } from 'src/app/shared/models/svcard/cardreissuance.req';
import { dxDataGridColumn } from 'devextreme/ui/data_grid';
import { BreadcrumbService } from 'src/app/services/breadcrumb.service';
@Component({
  selector: 'app-changeaccountcard-list',
  templateUrl: './changeaccountcard-list.component.html',
  styleUrl: './changeaccountcard-list.component.scss'
})
export class ChangeaccountcardListComponent implements OnInit {
  @ViewChild(DataGridControlComponent) dataGridControlComponent: DataGridControlComponent;
  req: CardReissuanceReq = new CardReissuanceReq();
  id = '';
  ...
  columns: Array<dxDataGridColumn | string> = [
    ...
    { dataField: 'referenceNo', caption: 'Số giao dịch', width: 150 },
    { dataField: 'custNo', caption: 'Mã khách hàng', width: 150 },
    { dataField: 'cardNo', caption: 'Số thẻ', width: 150 },
    { dataField: 'embossedName', caption: 'Tên trên thẻ', width: 200 },
    { dataField: 'authStat', caption: 'Trạng thái duyệt', width: 150 ,cellTemplate: (container, cellInfo) => {
      container.textContent = this.commonService.getNameById('AUTH_STATUS', cellInfo.data.authStat);
    },},
    { dataField: 'cardStat', caption: 'Trạng thái gửi', width: 150 ,cellTemplate: (container, cellInfo) => {
      container.textContent = this.commonService.getNameById('CARD_STATUS', cellInfo.data.cardStat);
    }, },
    { dataField: 'errorText', caption: 'Nội dung lỗi', width: 200, cssClass: 'text-danger' },
  ...
  loadData(params: any) {
    ...
    this.svcardService.getChangeAccountCardList(params).subscribe(
      (response: any) => {
  ...
  detail(rowData: any) {
    ...
    this.router.navigate([`${'/svcard/changeaccountcard'.split('?')[0]}`], { queryParams: { id: rowData.data.id } });
  }
  reload() {
    this.req = new CardReissuanceReq();
  ...
  create() {
    // this.id = '';
    // this.isPopupVisible = true;
    this.router.navigate([`${'/svcard/changeaccountcard'}`]);
  }
  hidePopup() {

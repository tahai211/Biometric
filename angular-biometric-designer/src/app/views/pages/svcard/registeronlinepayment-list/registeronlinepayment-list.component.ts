...
import { CardServiceReqDto } from 'src/app/shared/models/svcard/cardservice.req';
import { CONST } from 'src/app/shared/const/const';

@Component({
  selector: 'app-registeronlinepayment-list',
  templateUrl: './registeronlinepayment-list.component.html',
  styleUrl: './registeronlinepayment-list.component.scss'
})
export class RegisteronlinepaymentListComponent implements OnInit {
  @ViewChild(DataGridControlComponent) dataGridControlComponent: DataGridControlComponent;
  
  req: CardServiceReqDto = new CardServiceReqDto();
  id = '';
  ...
  dataStatusCard: any[] = CONST.dataCreditAuthStatus;
  dataServices: any[] =  CONST.dataCreditServices;
  dataProducts: any = [
  ...
  columns: Array<dxDataGridColumn | string> = [{
    ...
    cellTemplate: (container, cellInfo) => {
      container.textContent = this.commonService.getNameById('CREDIT_AUTH_STATUS', cellInfo.data.auth_stat);
    },
  }, {
  ...
  create() {
    this.router.navigate(['/svcard/registeronlinepayment-detail']);
  }

  loadData(params: any) {
    console.log('request filter: ', params);
    this.svcardService.getRegisterOnlinePaymentList(params).subscribe(
      (response: any) => {
  ...
  detail(rowData: any) {
    this.router.navigate(['/svcard/registeronlinepayment-detail'], { queryParams: { id: rowData.data.id } });
  }

  reload() {
    this.req = new CardServiceReqDto();

...
namespace Biometrics.ApiService.Controllers
{
    ...
    public class AuthenticationController : Controller
    {
        ...
        public AuthenticationController(IAuthenService userService,  IOptions<JWT> jwt)
        {
        ...
        public async Task<ActionResult<LoginResponseDTO>> Login([FromBody] LoginModel model)
        {
            ...
            {
                HttpBase httpResponseExtensions = null;
                var res = await _userService.Authentication(model.Username, model.Password, model.ServiceId);

                httpResponseExtensions = new HttpBase(HttpStatusCode.OK, res, ResposeType.ApplicationJson, "Success", "Login Success" ,false );
                return Ok(httpResponseExtensions);
            }
            catch (Exception ex)
            {
                var responeResult = new HttpBase(HttpStatusCode.BadRequest, ex.Message, ResposeType.TextPlain, "Fail", ex.Message, false);
                return BadRequest(responeResult);
            }
        }
        [Route("refresh-token")]
        [HttpPost]
        [ProducesResponseType(typeof(LoginResponseDTO), (int)HttpStatusCode.OK)]
        public async Task<ActionResult<LoginResponseDTO>> RefreshToken([FromBody] RefreshRequestModel request)
        {
            try
            {
                HttpBase httpResponseExtensions = null;
                var res = await _userService.RefreshToken(request.RefreshToken, request.ServiceId);
